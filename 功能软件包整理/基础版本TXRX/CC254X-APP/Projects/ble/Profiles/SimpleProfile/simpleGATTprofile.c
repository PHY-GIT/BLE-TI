...
static uint8 simpleProfileChar1Len = 0;
...
bStatus_t SimpleProfile_SetParameter( uint8 param, uint8 len, void *value )
{
  ...
  //uint16 read_handle_val=0;    //读取RX的头，用于广播发送给app 
  switch ( param )
  {
    ...
    case SIMPLEPROFILE_CHAR2:   //FFF2    Characteristic Value   ->5
        //read_handle_val=simpleProfileAttrTbl[5].handle;
        //NPI_PrintValue("FFF2-handle", read_handle_val, 16);   //0x28
        //NPI_PrintString("\n");
        if ( len == sizeof ( uint8 ) ) 
        {
...
{
  //NPI_PrintString("SimpleProfile_GetParameter\n");  //回车
    
  bStatus_t ret = SUCCESS;
  switch ( param )
  {
    case SIMPLEPROFILE_CHAR1:
      VOID osal_memcpy( value, simpleProfileChar1, simpleProfileChar1Len );
      *returnBytes = simpleProfileChar1Len;
      break;
    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR4_UUID:
        *pLen = 1;
        ...
        //NPI_PrintString("char 2 4\n");	
        break;

      case SIMPLEPROFILE_CHAR1_UUID:
        //NPI_PrintString("char1\n");	
        *pLen = simpleProfileChar1Len;
        VOID osal_memcpy( pValue, pAttr->pValue, simpleProfileChar1Len );
        break;
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      ...
      case GATT_CLIENT_CHAR_CFG_UUID:
        ...
        //if(pAttr->handle == simpleProfileAttrTbl[GUA_ATTRTBL_CHAR2_CCC_IDX].handle) 
